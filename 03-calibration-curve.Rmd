...
```{r, tukey-plot, echo = TRUE, eval = TRUE, warning = FALSE, message = FALSE, fig.height=4}
# Plot predicted absorbance vs residuals
plot(levm.fit)
```

Alternatively, one can use the `drc` package to fit a 4-PL curve using. However, the `drm` function performs a $\log_{10}(x)$ transformation by itself, so one get's results for $\log_{10}$-transformed concentrations.
...
Using the estimated parameters and the 4-PL equation, we can estimate the amount of IgG from the measured absorbance (in our case, we can only estimate a relative concentration).
