# Plot data {#plot-data}
For convience, we first define a plot theme:
...
plot.std <- function(
  title.hjust = 0, legend.pos = "right", legend.dir = "vertical", semilog = FALSE) {
  p <- ggplot(data = standard) +
    ...
    theme_plot(
      title.hjust = title.hjust, legend.pos = legend.pos, legend.dir = legend.dir
    )
...
Now let's have a look at our actual data. We measured IgG in serum of four donors on five different time points. We create a plot for our biological samples and then plot it together with the standard curve to have a better overview.
```{r, plot-samples, echo = TRUE, eval = TRUE, warning = FALSE, message = FALSE, chache = TRUE}
plot.donors <- function(
  title.hjust = 0, legend.pos = "right", legend.dir = "vertical", semilog = FALSE) {
  ggplot(
    data = donors,
    ...
    scale_color_viridis(
      ...
      discrete = TRUE,
      labels = paste("Donor", seq(n.donors)),
      guide = guide_legend(title = "")
    ) +
    scale_y_continuous(
      limits = c(min.absorb, max.absorb + 0.1 * max.absorb)
